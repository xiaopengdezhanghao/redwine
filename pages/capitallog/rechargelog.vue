<template>
	<view class="content">
		...
		<logMenu :current="1" :agent="agent"></logMenu>
		<!--充值记录-->
		<view class="uni-card">
			<view class="uni-card-content">
				<view class="uni-card-content-inner">
					...
					<view class="uni-list" hover-class="uni-list-cell-hover" v-for="(list,index) in lists" :key="index">
						<view class="uni-flex uni-row" style="justify-content: space-between;">
							<view class="text title" v-if="list.state == 0">等待付款<span v-if="agent == 1">({{list.username}})</span></view>
							<view class="text title recharge-color" v-if="list.state == 1">已完成<span v-if="agent == 1">({{list.username}})</span></view>
							<view class="text title withdraw-color" v-if="list.state == -1">已取消<span v-if="agent == 1">({{list.username}})</span></view>
							<view class="text uni-h4 red">{{list.amount}}</view>
						</view>
						<view class="uni-flex uni-row" style="justify-content: space-between;">
							<view class="text uni-h5">{{list.paytypetitle}}</view>
		...
		methods: {
			...
			getData(type = 1) {
				if (type == 1) { // //获取缓存的第一页数据
					const data = uni.getStorageSync('my_recharge_log_data');
				...
				this.$ajax.get({
					url: this.$service.api_lists.recharge_log,
					data: {
						type: this.type,
						page: this.page,
						agent:this.agent,
