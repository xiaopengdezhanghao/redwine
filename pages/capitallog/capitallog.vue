<template>
	<view class="content">
		...
		<logMenu :current="0" :agent="agent"></logMenu>
		<view class="uni-card">
			<view class="uni-card-content">
				<view class="uni-card-content-inner">
					...
					<view class="uni-list" hover-class="uni-list-cell-hover" v-for="(list,index) in lists" :key="index">
						<view class="uni-flex uni-row" style="justify-content: space-between;">
							<view class="text title">{{list.typename}}<span v-if="agent == 1">({{list.username}})</span></view>
							<view class="text uni-h4 red">{{parseInt(list.amountbefor) < parseInt(list.amountafter) ? '+' : '-'}}{{list.amount}}</view>
						</view>
						<view class="uni-flex uni-row" style="justify-content: space-between;">
							<view class="text uni-h5">结余：{{list.amountafter}}</view>
		...
		methods: {
			...
			getData(type = 1) {
				if (type == 1) { // //获取缓存的第一页数据
					const data = uni.getStorageSync('my_capital_data');
				...
				this.$ajax.get({
					url: this.$service.api_lists.capital_log,
					data: {
						type: this.type,
						page: this.page,
						agent:this.agent,
